  ...
  <q-page class="q-pa-xs-xs q-pa-sm-md" :style-fn="myTweak">
    ...
    <q-card class="q-pa-sm">
      <q-card-section class="text-center q-pa-sm q-mb-sm" :class="$q.dark.isActive ? 'bg-teal-7' : 'bg-teal-3'">
        <q-item-label class="text-h6">1차 역량평가하기 (팀원기준)</q-item-label>
      </q-card-section>
        ...
        <div class="col-xs-12 col-md-12 col-lg-4">
          <q-card class="q-pa-sm">
            <q-toolbar class="row q-px-none">
              ...
              <q-space />
              <q-btn v-if="sendCheck.lockBtn" outline color="blue-12" dense @click="saveDataLockSendSection" class="q-mr-xs">
...
onBeforeMount(() => {
  ...
  if (storeUser.setCatgCd <= '03') {
    setItemFg.value = '2021101'; // 팀장평가항목
...
const getData = async () => {
  try {
    const response = await api.post('/api/hce/hce1010_list', {
...
const getDataSelectList = async resRow => {
  try {
    const response = await api.post('/api/hce/hce1010_select_list', {
...
const saveDataAndHandleResult = resFormData => {
  api
    .post('/api/hce/hce1010_save', resFormData)
    .then(res => {
      let saveStatus = {};
