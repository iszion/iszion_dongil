  ...
  <q-page class="q-pa-xs-xs q-pa-sm-md" :style-fn="myTweak">
        ...
        <div class="col-xs-12 col-sm-12 col-lg-8">
          <q-banner rounded :class="$q.dark.isActive ? 'bg-grey-9' : 'bg-grey-3'" style="height: 100%">
            ...
            <div class="row">
              ...
              <div class="col-xs-12 col-sm-6">
                4. <span class="text-teal text-bold">각 포인 체크건수</span>는 <span class="text-teal text-bold">정해진 80%한도안에서 배정</span>에
                한해 <span class="text-teal text-bold">선택</span> 하실 수 있습니다.
                <br />
    ...
    <q-card class="q-pa-sm">
      <q-card-section class="text-center q-pa-sm q-mb-sm" :class="$q.dark.isActive ? 'bg-teal-7' : 'bg-teal-3'">
        <q-item-label class="text-h6">1차 역량평가하기 (평균기준) </q-item-label>
      </q-card-section>
...
const rowData = reactive({ rows: [], rowSel: [] });
...
function revertDataSet(resData) {
  for (let i = 0; i < rowData.rowsSel.length; i++) {
    if (rowData.rowsSel[i].evtEmpCd === resData.evtEmpCd) {
      rowData.rowsSel[i].markCh = rowData.rowsSel[i].markChOld;
      rowData.rowsSel[i].markPoint = rowData.rowsSel[i].markPointOld;
    }
  }
  let saveStatus = {};
...
function saveDataSet(resData) {
  for (let i = 0; i < rowData.rowsSel.length; i++) {
    if (rowData.rowsSel[i].evtEmpCd === resData.evtEmpCd) {
      rowData.rowsSel[i].markChOld = rowData.rowsSel[i].markCh;
      rowData.rowsSel[i].markPointOld = rowData.rowsSel[i].markPoint;
    }
  }
}
...
const percentOverCheck = resData => {
  ...
  const checkPer = rowData.rowsSel.reduce((sum, row) => sum + row.markPoint, 0) / maxInwon;
  if (maxInwon === 1) {
    saveDataSet(resData);
    return true;
  } else if (maxInwon === 2) {
