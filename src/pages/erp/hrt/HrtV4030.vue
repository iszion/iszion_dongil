<template>
  <q-page class="q-pa-xs-xs q-pa-sm-md" :style-fn="myTweak">
    ...
    <!--          <hrt-v4030p :rowData="rowData.rows" @close="handleClose" />-->
    <!--        </q-card-section>-->
    <!--      </q-card>-->
    <!--    </q-dialog>-->
  </q-page>
      ...
      children: [
        {
          ...
          minWidth: 100,
          cellStyle: params => {
            return getStatusMessageStyle1(params.data);
          },
        },
        {
          headerName: '직무역량',
          ...
          minWidth: 100,
          cellStyle: params => {
            return getStatusMessageStyle2(params.data);
          },
        },
      ],
    },
...
const getStatusMessageStyle1 = val => {
  // console.log(JSON.stringify(val));
...
const getStatusMessageStyle2 = val => {
  // console.log(JSON.stringify(val));
...
const getData = async () => {
  try {
    const response = await api.post('/api/hrt/hrt4030_list', {
...
const headerGroup = reactive({
  ...
  headRow1: [
    ...
    { name: '개인편차의제곱', rowspan: 1, colspan: 2 },
    { name: '소속의편차평균(분산)', rowspan: 1, colspan: 2 },
    { name: '분산의루트(표준편차)', rowspan: 1, colspan: 2 },
    { name: '전체평균에대한편차', rowspan: 1, colspan: 2 },
    { name: '소속표준편차', rowspan: 1, colspan: 2 },
    { name: '역량조정점수', rowspan: 1, colspan: 2 },
...
const excelExport = (data, options) => {
  ...
  XLSX.utils.book_append_sheet(wb, ws, '최종(편차적용)_종합');
  XLSX.writeFile(wb, '최종(편차적용)_종합.xlsx');
};
/* ************************************************************************* *
 ** Excel저장  처리부분 끝
 ** ************************************************************************* */
