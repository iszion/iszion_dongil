        ...
        <q-card-section class="q-pt-none">
          ...
          <div id="printZone">
            <div class="row q-px-xs">
              <span class="text-h6">목표성과평가 현황</span>
              <q-space />
            ...
            <table>
              <tr>
                ...
                <th rowspan="1" colspan="2">내용</th>
                <th rowspan="2" colspan="1">평가기준</th>
                <th rowspan="2" colspan="1">가중치</th>
              ...
              <tr v-for="data in props.rowData" :key="data.empCd">
                ...
                <td>{{ data.titlNm }}</td>
                <td>{{ data.empNm }}</td>
                <td>{{ data.targetDoc }}</td>
                <td>{{ data.workDoc }}</td>
                <td>{{ data.evaNm }}</td>
                <td>{{ data.weight }}</td>
                <td>{{ data.selfPoint }}</td>
                <td>{{ data.selfPointX }}</td>
                <td>{{ data.markPoint }}</td>
                <td>{{ data.markPointX }}</td>
              </tr>
            </table>
          </div>
        </q-card-section>
...
const headerGroup = reactive({
  header: [],
  headProps: ['deptNm', 'titlNm', 'empNm', 'targetDoc', 'workDoc', 'evaNm', 'weight', 'selfPoint', 'selfPointX', 'markPoint', 'markPointX'],
  headRow1: [
    { name: '소속팀', rowspan: 1, colspan: 2 },
    { name: '성명', rowspan: 2, colspan: 1, key: 'empNm' },
    { name: '평가내용', rowspan: 1, colspan: 2 },
    { name: '평가기준', rowspan: 2, colspan: 1, key: 'evaNm' },
    { name: '가중치', rowspan: 2, colspan: 1, key: 'weight' },
...
const excelExport = (data, options) => {
  ...
  XLSX.writeFile(wb, '성과평가현황.xlsx');
};
</script>

<style scoped>
@import 'src/css/print/hrt1010.css';
</style>
