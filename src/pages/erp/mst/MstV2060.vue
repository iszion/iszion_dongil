      ...
      <div class="col-12">
        <q-card bordered>
          ...
          <q-bar class="q-px-sm">
            ...
            <span class="text-subtitle2 q-px-sm">평가대상그룹리스트</span>
          </q-bar>
...
const addDataSection = () => {
  ...
  // 첫컬럼에 focus
  gridApi.value.setFocusedCell(addIndex, 'evtgCd');
};
//****  신규 자료 추가부분 끝    ******************************//
...
const saveDataSection = () => {
  ...
  for (let i = 0; i < rowData.rows.length; i++) {
    if (!isEmpty(rowData.rows[i].evtgCd)) {
  ...
  for (let i = 0; i < updateData.value.length; i++) {
    if (!isEmpty(updateData.value[i].evtgCd)) {
...
const getData = async () => {
  try {
    const response = await api.post('/api/mst/mst2060_list', { paramSetYear: setYearGroup.value.setYear }, { headers: authHeader() });
...
const saveDataAndHandleResult = resFormData => {
  api
    .post('/api/mst/mst2060_save', resFormData, { headers: authHeader() })
    .then(res => {
      let saveStatus = {};
      if (res.data.rtn === '0') {
        saveStatus.rtn = 1;
