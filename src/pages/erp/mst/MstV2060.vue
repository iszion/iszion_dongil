...
const addDataSection = () => {
  ...
  // 첫컬럼에 focus
  myGrid.value.api.setFocusedCell(addIndex, 'evtgCd');
};
//****  신규 자료 추가부분 끝    ******************************//
...
const getData = async () => {
  try {
    const response = await api.post('/api/mst/mst2060_list', { paramSetYear: storeYear.setYear });
...
const saveDataAndHandleResult = resFormData => {
  api
    .post('/api/mst/mst2060_save', resFormData)
    .then(res => {
      let saveStatus = {};
